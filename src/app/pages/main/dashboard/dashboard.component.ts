...
import { Component } from '@angular/core';
import { DomSanitizer, SafeResourceUrl } from '@angular/platform-browser';
import { FileUploadService } from '../../../service/file-upload.service'; // Adjust path if needed
import { forkJoin, Observable, of } from 'rxjs';
  ...
  uploadFiles() {
    ...
    const uploadObservables: Observable<any>[] = this.files
      ...
      .map(previewFile => {
        ...
        return this.fileUploadService
          .uploadToN8n(previewFile.file, 'currentUser', previewFile.type)
          .pipe(
            tap(() => {
              previewFile.status = 'uploaded';
              console.log(`✅ Successfully uploaded to n8n: ${previewFile.name}`);
            }),
            catchError(error => {
              console.error(`❌ Upload failed for: ${previewFile.name}`, error);
              previewFile.status = 'failed';
    ...
    forkJoin(uploadObservables).subscribe({
      next: (results) => {
        this.isUploading = false;
        const successfulUploads = results.filter(r => r !== null).length;
        alert(`Successfully uploaded ${successfulUploads} / ${uploadObservables.length} file(s) to n8n!`);
      ...
      error: () => {
        this.isUploading = false;
        alert('An unexpected error occurred while uploading to n8n. Please try again.');
      }
    });
  }
}
